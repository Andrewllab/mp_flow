...
class TTDataset(Dataset):
    ...
    def __init__(self,
        ...
        # load small state_action data into cache
        # self.states_actions_data = dict(np.load(self.root_dir / "recording.npz", allow_pickle=True))
        self.states_actions_data = {
            k: v.tolist() if hasattr(v, "tolist") else v for k,v in np.load(self.root_dir / "recording.npz", allow_pickle=True).items()
        }
    ...
    def _get_sequence(self, index, fore_size, back_size=0):
        ...
        index_, right_bound, ep_idx = self._get_episode_index(index)

        # st = time.time()
        irecord_path = os.path.join(self.images_dir, f"record_{ep_idx}.npz")
        with np.load(irecord_path, mmap_mode="r") as img_data:
            seq_dict["rgb_obs"]["rgb_static"] = torch.from_numpy(img_data["left_cam"][index_]).permute(2,0,1)
            seq_dict["rgb_obs"]["rgb_gripper"] = torch.from_numpy(img_data["right_cam"][index_]).permute(2,0,1)
        # ed = time.time()
        # print(f"image acces time: {ed - st}s")

        # st = time.time()
        obs = self.states_actions_data["obs"]["bat1"]
        robo_pos = torch.from_numpy(obs["xpos"][index])
        robo_quat = torch.from_numpy(obs["xquat"][index])
        seq_dict["robot_obs"] = torch.cat([robo_pos, robo_quat], dim=0)[None,...]

        index_right = index + fore_size if index + fore_size-1 <=right_bound else right_bound+1
        acts = self.states_actions_data["input_data"]
        acts_pos = torch.from_numpy(np.array(acts["pos"][index:index_right]))
        acts_euler = torch.from_numpy(np.array(acts["rot"][index:index_right]))
        seq_dict["actions"] = torch.cat([acts_pos, acts_euler], dim=1)
        # ed = time.time()
        # print(f"action acces time: {ed - st}s")

        # st = time.time()
        seq_dict = self.apply_transform(seq_dict)
        # ed = time.time()
        # print(f"transform time: {ed - st}s")
