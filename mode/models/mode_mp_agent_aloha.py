...
from mode.models.perceptual_encoders.pretrained_resnets import \
    FiLMResNet34Policy, FiLMResNet50Policy
...
def print_model_parameters(model):
    ...
    for name, submodule in model.named_modules():
        ...
        if '.' not in name or name.count(
                '.') <= 10:  # Can be adjusted based on your model structure
...
class MoDEMPAgent(pl.LightningModule):
    """
    ...
    def __init__(
        ...
        super(MoDEMPAgent, self).__init__()
        ...
        # goal encoders
        self.language_goal = hydra.utils.instantiate(
            language_goal) if language_goal else None
    ...
    def load_pretrained_parameters(self, ckpt_path, strict: bool = False):
        ...
        try:
            ...
            if os.path.isdir(ckpt_path):
                cleaned_safetensors = os.path.join(ckpt_path,
                ...
                if os.path.exists(cleaned_safetensors):
                    ...
                    raise FileNotFoundError(
                        f"No cleaned weights found in {ckpt_path}")
            else:
            ...
            def process_state_dict_entry(key, checkpoint_tensor, current_shape):
                ...
                if expanded is not None:
                    print(
                        f"Reshaped {key} from {checkpoint_tensor.shape} to {current_shape}")
                    return expanded

                # For batch norm running stats
                if 'running_' in key and len(checkpoint_tensor.shape) != len(
                        current_shape):
            ...
            for key, checkpoint_tensor in state_dict.items():
                ...
                if target_key in current_state:
                    ...
                    processed_tensor = process_state_dict_entry(target_key,
                                                                checkpoint_tensor,
                                                                current_shape)
            ...
            raise RuntimeError(
                f"Failed to load weights from {ckpt_path}: {str(e)}")
    ...
    def configure_optimizers(self):
        ...
        if self.use_perceiver:
            optim_groups.extend([
                {"params": self.perceiver.parameters(),
                 "weight_decay": self.optimizer_config.transformer_weight_decay},
            ])
    ...
    def on_before_zero_grad(self, optimizer=None):
            ...
            if p.grad is not None:
                ...
                if 'blocks' in name:
                    ...
                    layer_name = '.'.join(parts[
                                          2:])  # Join the rest of the parts to get the layer name
        ...
        layer_grad_norms['input_layers'] = layer_grad_norms[
                                               'input_layers'] ** 0.5
        ...
        self.log("debug/input_layers_grad_norm",
                 layer_grad_norms['input_layers'], on_step=True, on_epoch=False,
                 sync_dist=True)

        # Log block and layer-wise grad norms
        for block, layers in layer_grad_norms['blocks'].items():
            for layer, norm in layers.items():
                self.log(f"debug/block_{block}_{layer}_grad_norm", norm,
    ...
    def get_optim_groups(self):
        ...
        def use_weight_decay(name):
            return all(
                x not in name for x in ['bias', 'LayerNorm', 'embedding'])
    ...
    def training_step(self, batch: Dict[str, Dict],
        ...
        normalizer_bound_min = self.mp.w_min.reshape(14, -1)
        normalizer_bound_max = self.mp.w_max.reshape(14, -1)
        for i in range(14):
        ...
        if self.entropy_gamma > 0:
            self.log("train/load_balancing_loss", entropy_loss, on_step=False,
                     on_epoch=True, sync_dist=True, batch_size=total_bs)
        if self.router_z_delta > 0:
            self.log("train/router_z_delta", router_z_loss, on_step=False,
                     on_epoch=True, sync_dist=True, batch_size=total_bs)
        return total_loss

    @torch.no_grad()
    def validation_step(self, batch: Dict[str, Dict],
                        batch_idx: int) -> None:  # Change return type to None
        output = {}
        dataset_batch = batch
        perceptual_emb, latent_goal = self.compute_input_embeddings(
            dataset_batch)
    ...
    def log_expert_usage(self, model, epoch):
        ...
        if expert_usages:
            ...
            sns.heatmap(
                ...
                yticklabels=[f'blocks.{i}' for i in
                             range(expert_usage_data_normalized.shape[0])]
            )
    ...
    def _log_validation_metrics(self, pred_loss):
        ...
        """
        self.log(f"val_act/{self.modality_scope}_act_loss_pp", pred_loss,
                 sync_dist=True)
    ...
    def compute_input_embeddings(self, dataset_batch):
        ...
        if self.use_text_not_embedding:
            ...
            latent_goal = self.lang_buffer.get_goal_instruction_embeddings(
                dataset_batch["lang_text"]).to(rgb_static.dtype)
        else:
            latent_goal = self.language_goal(dataset_batch["lang"]).to(
                rgb_static.dtype)

        # perceptual_emb = self.embed_visual_obs(rgb_static, rgb_gripper,
        #                                        latent_goal)
        perceptual_emb = self.embed_visual_obs(rgb_static,
                                               latent_goal)
    ...
    def embed_visual_obs(self, rgb_static, latent_goal):
        ...
        static_tokens = einops.rearrange(static_tokens, '(b t) d -> b t d',
                                         b=rgb_static.shape[0])
        # gripper_tokens = einops.rearrange(gripper_tokens, '(b t) d -> b t d',
        #                                   b=rgb_gripper.shape[0])
        # token_seq = torch.cat([static_tokens, gripper_tokens], dim=1)
        token_seq = static_tokens
    ...
    def forward(self, obs, goal):
        ...
        if self.use_text_not_embedding:
            ...
            latent_goal = self.language_goal(goal["lang"]).unsqueeze(0).to(
                torch.float32).to(obs["rgb_obs"]['rgb_static'].device)
        ...
        perceptual_emb = self.embed_visual_obs(rgb_static,
                                               latent_goal)
        if self.use_proprio:
            perceptual_emb['robot_obs'] = obs['robot_obs']
    ...
    def precompute_expert_for_inference(self, goal=None):
        logger.info("Precomputing experts with sampling steps %d",
                    self.num_sampling_steps)
        sigmas = self.get_noise_schedule(self.num_sampling_steps,
                                         self.noise_scheduler)[:-1]
    ...
    def on_train_epoch_end(self,
                           unused: Optional = None) -> None:  # type: ignore
    ...
    def make_sample_density(self):
        ...
        if self.sigma_sample_density_type == 'loglogistic':
            loc = sd_config['loc'] if 'loc' in sd_config else math.log(
                self.sigma_data)
            scale = sd_config['scale'] if 'scale' in sd_config else 0.5
            min_value = sd_config[
                'min_value'] if 'min_value' in sd_config else self.sigma_min
            max_value = sd_config[
                'max_value'] if 'max_value' in sd_config else self.sigma_max
            return partial(utils.rand_log_logistic, loc=loc, scale=scale,
        ...
        if self.sigma_sample_density_type == 'loguniform':
            ...
            max_value = sd_config[
                'max_value'] if 'max_value' in sd_config else self.sigma_max
            return partial(utils.rand_log_uniform, min_value=min_value,
                           max_value=max_value)

        if self.sigma_sample_density_type == 'uniform':
            return partial(utils.rand_uniform, min_value=self.sigma_min,
        ...
        if self.sigma_sample_density_type == 'discrete':
            sigmas = self.get_noise_schedule(self.num_sampling_steps * 1e5,
                                             'exponential')
            return partial(utils.rand_discrete, values=sigmas)
        if self.sigma_sample_density_type == 'split-lognormal':
            loc = sd_config['mean'] if 'mean' in sd_config else sd_config['loc']
            scale_1 = sd_config['std_1'] if 'std_1' in sd_config else sd_config[
                'scale_1']
            scale_2 = sd_config['std_2'] if 'std_2' in sd_config else sd_config[
                'scale_2']
            return partial(utils.rand_split_log_normal, loc=loc,
    ...
    def denoise_actions(  # type: ignore
        ...
        if len(latent_goal.shape) < len(
                perceptual_emb['state_images'].shape if isinstance(
                        perceptual_emb, dict) else perceptual_emb.shape):
    ...
    def reset_expert_cache(self):
        self.model.inner_model.reset_all_caches() if hasattr(
            self.model.inner_model, 'reset_all_caches') else None
    ...
    def sample_loop(
        ...
        use_scaler = extra_args[
            'use_scaler'] if 'use_scaler' in extra_args else False
        ...
        # ODE deterministic
        if sampler_type == 'lms':
            x_0 = sample_lms(self.model, state, x_t, goal, sigmas,
                             scaler=scaler, disable=True,
                             extra_args=reduced_args)
        # ODE deterministic can be made stochastic by S_churn != 0
        elif sampler_type == 'heun':
            x_0 = sample_heun(self.model, state, x_t, goal, sigmas,
                              scaler=scaler, s_churn=s_churn, s_tmin=s_min,
                              disable=True)
        # ODE deterministic
        elif sampler_type == 'euler':
            x_0 = sample_euler(self.model, state, x_t, goal, sigmas,
                               scaler=scaler, disable=True)
        # SDE stochastic
        elif sampler_type == 'ancestral':
            x_0 = sample_dpm_2_ancestral(self.model, state, x_t, goal, sigmas,
                                         scaler=scaler, disable=True)
            # SDE stochastic: combines an ODE euler step with an stochastic noise correcting step
        elif sampler_type == 'euler_ancestral':
            x_0 = sample_euler_ancestral(self.model, state, x_t, goal, sigmas,
                                         scaler=scaler, disable=True)
        # ODE deterministic
        elif sampler_type == 'dpm':
            x_0 = sample_dpm_2(self.model, state, x_t, goal, sigmas,
                               disable=True)
        # ODE deterministic
        elif sampler_type == 'dpm_adaptive':
            x_0 = sample_dpm_adaptive(self.model, state, x_t, goal,
                                      sigmas[-2].item(), sigmas[0].item(),
                                      disable=True)
        # ODE deterministic
        elif sampler_type == 'dpm_fast':
            x_0 = sample_dpm_fast(self.model, state, x_t, goal,
                                  sigmas[-2].item(), sigmas[0].item(),
                                  len(sigmas), disable=True)
        # 2nd order solver
        elif sampler_type == 'dpmpp_2s_ancestral':
            x_0 = sample_dpmpp_2s_ancestral(self.model, state, x_t, goal,
                                            sigmas, scaler=scaler, disable=True)
        # 2nd order solver
        elif sampler_type == 'dpmpp_2m':
            x_0 = sample_dpmpp_2m(self.model, state, x_t, goal, sigmas,
                                  scaler=scaler, disable=True)
        elif sampler_type == 'dpmpp_2m_sde':
            x_0 = sample_dpmpp_sde(self.model, state, x_t, goal, sigmas,
                                   scaler=scaler, disable=True)
        elif sampler_type == 'ddim':
            x_0 = sample_ddim(self.model, state, x_t, goal, sigmas,
                              scaler=scaler, disable=True)
        elif sampler_type == 'dpmpp_2s':
            x_0 = sample_dpmpp_2s(self.model, state, x_t, goal, sigmas,
                                  scaler=scaler, disable=True)
        elif sampler_type == 'debugging':
            x_0 = sample_dpmpp_2_with_lms(self.model, state, x_t, goal, sigmas,
                                          scaler=scaler, disable=True)
    ...
    def get_noise_schedule(self, n_sampling_steps, noise_schedule_type):
        ...
        if noise_schedule_type == 'karras':
            return get_sigmas_karras(n_sampling_steps, self.sigma_min,
                                     self.sigma_max, 7,
                                     self.device)  # rho=7 is the default from EDM karras
        elif noise_schedule_type == 'exponential':
            return get_sigmas_exponential(n_sampling_steps, self.sigma_min,
                                          self.sigma_max, self.device)
        elif noise_schedule_type == 'vp':
            return get_sigmas_vp(n_sampling_steps, device=self.device)
        elif noise_schedule_type == 'linear':
            return get_sigmas_linear(n_sampling_steps, self.sigma_min,
                                     self.sigma_max, device=self.device)
            ...
            return get_sigmas_ve(n_sampling_steps, self.sigma_min,
                                 self.sigma_max, device=self.device)
        elif noise_schedule_type == 'iddpm':
            return get_iddpm_sigmas(n_sampling_steps, self.sigma_min,
                                    self.sigma_max, device=self.device)
