...
def normalize(tensor, mean, sigma):
    normalized = (tensor - mean)/(sigma + 1e-6)
    return normalized

def unnormalize(normalized_tensor, mean, sigma):
    tensor = normalized_tensor *(sigma + 1e-6) + mean
    return tensor
...
class BSpline(torch.nn.Module):
    ...
    def __init__(self, num_dof, num_basis=10, degree_p=4, dtype=torch.float32,
        ...
        w_mean = kwargs.get("w_mean", np.array([0] *num_dof))
        w_sigma = kwargs.get("w_sigma", np.array([1] * num_dof))

        self.register_buffer("w_mean", torch.tensor(w_mean))
        self.register_buffer("w_sigma", torch.tensor(w_sigma))
    ...
    def traj_to_params(self, action_sequences, update_bounds: bool = False, **kwargs):
        ...
        params = normalize(params, self.w_mean, self.w_sigma)
    ...
    def get_traj(self, para, **kwargs):
        ...
        params = para["params"]
        params = unnormalize(params, self.w_mean, self.w_sigma)
        params = torch.einsum('...ji->...ij', params)
